...
function wpappninja_get_menu_reloaded($locale = 'en', $translate = false) {
	...
	foreach($items as $item) {
		...
		if ($doingapi && isset($_SERVER['HTTP_X_WPAPPNINJA_IOS'])) {
			...
			if ($item['type'] == 'link' && substr($item['id'], 0, 4) == 'http') {
				...
				if (preg_match('/^'.preg_quote(get_bloginfo('url') . '/', "/").'/', $item['id'])) {
		...
		if (get_wpappninja_option('speed') == '1') {
			...
			if ($item['id'] == '-999') {
				$item['type'] = 'link';
				$item['id'] = get_bloginfo('url') . '/' . '?wpapp_shortcode=wpapp_config';
			}

			if ($item['id'] == '-100') {
				$item['type'] = 'link';
				$item['id'] = get_bloginfo('url') . '/' . '?wpapp_shortcode=wpapp_history';
			}

			if ($item['id'] == '0') {
				$item['type'] = 'link';
				$item['id'] = get_bloginfo('url') . '/' . '?wpapp_shortcode=wpapp_recent';
			}
	...
	if (get_wpappninja_option('qrcode') == '1' && get_wpappninja_option('speed') == '1' && 1<0) {
		$menu[] = array(
					'id' => get_bloginfo('url') . '/' . "?wpappqrcode=1",
					'name' => __("Scan code", "wpappninja"),
					'type' => "link",
					'icon' => "camera",
					'feat' => "0"
	...
	if (count($pages) == 0) {
		...
		foreach ($pages as $k => $page) {
			...
			$pages[$k]['icon_2'] = preg_replace('#_fill$#', '', $pages[$k]['icon']) . "_fill";
   			...
   			$pages[$k]['icon'] = preg_replace('#^social_#', 'logo_', $pages[$k]['icon']);
			$pages[$k]['icon_2'] = preg_replace('#^social_#', 'logo_', $pages[$k]['icon']);
