...
if (isset($_POST)) {
    ...
    if (isset($_POST['wpappninja_app_name']) && isset($_POST['wpappninja_app_logo']) && check_admin_referer('wpappninja_firststep')) {

        $_options['app']['name'] = esc_attr(sanitize_text_field($_POST['wpappninja_app_name']));
        $_options['app']['logo'] = esc_attr(sanitize_text_field($_POST['wpappninja_app_logo']));

        $_options['app']['splashscreen'] = esc_attr(sanitize_text_field($_POST['wpappninja_app_logo']));
    }
...
if (!wpappninja_is_store_ready()) { ?>
    ...
    <form action="" method="post" style="margin-bottom:50px;padding:40px;background:white;position: absolute;width:60%;box-shadow: 0 0 18px #999;z-index:1001;max-width:500px;height: auto;/* min-height:300px; */margin:auto;overflow: auto;top: 50px;/* bottom: 0; */left: 0;right: 0;">
	    <?php wp_nonce_field('wpappninja_firststep'); ?>
...
if (current_user_can( wpappninja_get_right() ) ) {
    ...
    if ((get_wpappninja_option('package', '') != '' && (get_option('wpmobile_firebase_config', '') == '' || get_option('wpappninja_google_json', '') == '')) || (get_wpappninja_option('appstore_package', '') != '' && get_wpappninja_option('appstore_package', '') != 'xxx' && get_wpappninja_option('appstore_package', '') != '' && get_option('wpappninja_pem_file', '') == '')) { ?>
    <div class="wpappninja_help" style="border-left: 5px solid #c10033;background: #ffffd8;margin-bottom: 26px;"><?php _e('Push notifications are not yet fully configured', 'wpappninja'); echo ' <a style="display: inline-block;margin-left: 17px;font-size:17px;" href="?page=' . WPAPPNINJA_PUSH_SLUG . '&settings&needConfiguration=true">';?><?php echo strtolower(__('CONFIGURE', 'wpappninja'));?></a></div>
<?php }
